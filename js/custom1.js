$(function () {
  $(document).mousemove(function (e) {
    let x = e.pageX;
    let y = e.pageY;
    $('.cursor').show().css({
      left: x,
      top: y
    });
  })
  $('header .bars').click(function () {
    $('nav').toggleClass('on');
  })

  $('.wrap section:visible').on('wheel', function (e) {
    let sectionIndex = $(this).index();

    if (sectionIndex < 2) {
      // 첫 번째와 두 번째 섹션은 기본 스크롤 동작을 허용
      return;
    } else {
      // 세 번째 섹션부터 애니메이션 효과 적용
      e.preventDefault();
      let nav;
      if (e.originalEvent.wheelDelta > 0 || e.originalEvent.detail < 0) {
        nav = $(this).prev();
      } else {
        //console.log($(this).attr('class'))
        if($(this).hasClass('menu4')){
          nav = $('footer');
        }else{
          nav = $(this).next();
        }
       
      }

      if (nav.length) {
        let moveTop = nav.offset().top;
        $('html,body').stop().animate({
          scrollTop: moveTop,
        }, 500, /* 'easeInOutBounce' */);
        /* console.log(nav) */
      }
    }
  });


  let sc;
  let sectionI = $('.wrap section').length;

  $(window).scroll(function () {
    let sc = $(window).scrollTop();
    let windowHeight = $(window).height();
    let ht1 = windowHeight * 2; // 첫 번째 섹션 높이 (200vh)
    let ht2 = windowHeight; // 나머지 섹션 높이 (100vh)
    let sectionCount = $('.wrap section').length;

    for (let i = 0; i < sectionCount; i++) {
      var start = i === 0 ? 0 : ht1 + ht2 * (i - 1);
      var end = i === 0 ? ht1 : ht1 + ht2 * i;

      if (sc >= start && sc < end) {
        $('.wrap section').eq(i).addClass('on').siblings().removeClass('on');
        $('.slide_nav_all .slide_nav').eq(i).addClass('on').siblings().removeClass('on');
      }
    }
  });



  $('.slide_nav_all .slide_nav').click(function () {
    let i = $(this).index();
    let ht1 = $(window).height() * 2; // 첫 번째 섹션 높이 (200vh)
    let ht2 = $(window).height(); // 나머지 섹션 높이 (100vh)
    let scrollTopValue;

    if (i === 0) {
      scrollTopValue = 0; // 첫 번째 섹션의 경우
    } else {
      scrollTopValue = ht1 + ht2 * (i - 1); // 나머지 섹션의 경우
    }

    $('html, body').stop().animate({
      scrollTop: scrollTopValue
    }, 1600, /* 'easeInOutBounce' */);
  });

});

window.addEventListener('scroll', function () {
  const scrollTop = window.scrollY || document.documentElement.scrollTop;
  const bluecircle = document.querySelector('.bluecircle');
  const mainSection = document.querySelector('main'); // main 섹션 선택
  const speed = 0.2; // 스크롤 속도 조절

  if (bluecircle && mainSection) { // 요소가 존재하는지 확인
    const mainRect = mainSection.getBoundingClientRect();
    const mainTop = mainRect.top + window.scrollY;
    const mainBottom = mainRect.bottom + window.scrollY;

    // 메인 섹션 내에 스크롤이 위치한 경우에만 애니메이션 적용
    if (scrollTop >= mainTop && scrollTop <= mainBottom) {
      bluecircle.style.transform = `translateY(${(scrollTop - mainTop) * speed}px)`;
    } else {
      bluecircle.style.transform = 'translateY(0)'; // 섹션을 벗어나면 초기 상태로
    }
  }
});

document.addEventListener("DOMContentLoaded", function () {
  const pinkcircle = document.querySelector('.pinkcircle');
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        pinkcircle.classList.add('animate');
      } else {
        pinkcircle.classList.remove('animate');
      }
    });
  }, {
    threshold: 0.5 // 요소가 50% 이상 보이면 콜백 실행
  });

  observer.observe(document.querySelector('.second_main'));
});

/* greencircle */
/* document.addEventListener('mousemove', function (event) {
  const greencircle = document.querySelector('.greencircle');
  const windowWidth = window.innerWidth;
  const mouseX = event.clientX;
  const offsetX = (mouseX - windowWidth / 2) / windowWidth * 20; // 이동 범위 조정

  greencircle.style.transform = `translate(-50%, -50%) translateX(${offsetX}px)`;
}); */

//java 타이핑 애니메이션
/* document.addEventListener("DOMContentLoaded", function () {
  const typewriterElements = document.querySelectorAll('main .me_introduce .typewriter');

  typewriterElements.forEach((element, index) => {
    const text = element.textContent;
    element.textContent = ''; // 초기 텍스트를 비웁니다.

    setTimeout(() => {
      typeWriter(element, text);
    }, index * 800); // 각 애니메이션의 시작을 지연시킵니다.
  });

  function typeWriter(element, text, i = 0) {
    if (i < text.length) {
      element.innerHTML += text.charAt(i);
      setTimeout(() => typeWriter(element, text, i + 1), 50); // 각 글자의 타이핑 속도를 조절합니다.
    }
  }
}); */
/* document.addEventListener("DOMContentLoaded", function () {
  const typewriterElements = document.querySelectorAll('main .me_introduce .typewriter');

  function typeWriter(element, text, i = 0) {
    if (i < text.length) {
      element.innerHTML += text.charAt(i);
      setTimeout(() => typeWriter(element, text, i + 1), 50); // 각 글자의 타이핑 속도를 조절합니다.
    }
  }

  typewriterElements.forEach((element, index) => {
    const text = element.textContent;

    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          element.textContent = ''; // 요소가 보일 때 텍스트를 초기화
          setTimeout(() => {
            typeWriter(element, text);
          }, 800 * index); // 각 애니메이션의 시작을 지연시킵니다.
        }
      });
    }, {
      threshold: 0.5 // 요소가 50% 이상 보이면 콜백 실행
    });

    observer.observe(element);
  });
}); */

//java 타이핑 애니메이션
document.addEventListener("DOMContentLoaded", function () {
  const typewriterElements = document.querySelectorAll('main .me_introduce .typewriter');

  function typeWriter(element, text, i = 0) {
    if (i < text.length) {
      element.innerHTML = text.slice(0, i + 1); // 요소의 텍스트를 인덱스까지 설정
      setTimeout(() => typeWriter(element, text, i + 1), 50); // 각 글자의 타이핑 속도를 조절합니다.
    }
  }

  typewriterElements.forEach((element, index) => {
    const text = element.innerHTML; // innerHTML로 텍스트를 가져옵니다.

    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          element.innerHTML = ''; // 요소가 보일 때 텍스트를 초기화
          setTimeout(() => {
            typeWriter(element, text);
          }, 800 * index); // 각 애니메이션의 시작을 지연시킵니다.
        }
      });
    }, {
      threshold: 0.5 // 요소가 50% 이상 보이면 콜백 실행
    });

    observer.observe(element);
  });
});

/* orange circle */


/* footer */
window.addEventListener('scroll', function () {
  const footer = document.querySelector('footer');
  const yellowCircle = document.querySelector('.yellow_circle');
  const skyblueCircle = document.querySelector('.skyblue_circle');
  const footerOffsetTop = footer.offsetTop;
  const windowHeight = window.innerHeight;
  const scrollY = window.scrollY;

  if (scrollY + windowHeight > footerOffsetTop) {
    yellowCircle.style.right = '0';
    yellowCircle.style.top = '-100px';
    skyblueCircle.style.left = '-110px';
    skyblueCircle.style.bottom = '-709px';
  }
});
